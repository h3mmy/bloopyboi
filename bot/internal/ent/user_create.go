...
func (uc *UserCreate) AddMediaRequestIDs(ids ...int) *UserCreate {
	uc.mutation.AddMediaRequestIDs(ids...)
	return uc
}
...
func (uc *UserCreate) AddMediaRequest(m ...*MediaRequest) *UserCreate {
	...
	return uc.AddMediaRequestIDs(ids...)
}
...
func (uc *UserCreate) defaults() {
	if _, ok := uc.mutation.Discordid(); !ok {
		v := user.DefaultDiscordid
		uc.mutation.SetDiscordid(v)
	}
	if _, ok := uc.mutation.Plexid(); !ok {
		v := user.DefaultPlexid
		uc.mutation.SetPlexid(v)
	}
}

// check runs all checks and user-defined validators on the builder.
func (uc *UserCreate) check() error {
	if _, ok := uc.mutation.BloopyId(); !ok {
		return &ValidationError{Name: "bloopyId", err: errors.New(`ent: missing required field "User.bloopyId"`)}
	}
	if _, ok := uc.mutation.Discordid(); !ok {
		return &ValidationError{Name: "discordid", err: errors.New(`ent: missing required field "User.discordid"`)}
	}
	if _, ok := uc.mutation.Plexid(); !ok {
		return &ValidationError{Name: "Plexid", err: errors.New(`ent: missing required field "User.Plexid"`)}
	}
	if _, ok := uc.mutation.Bloopnetid(); !ok {
		return &ValidationError{Name: "bloopnetid", err: errors.New(`ent: missing required field "User.bloopnetid"`)}
	}
	if _, ok := uc.mutation.Authentikpkid(); !ok {
		return &ValidationError{Name: "authentikpkid", err: errors.New(`ent: missing required field "User.authentikpkid"`)}
	}
	return nil
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if value, ok := uc.mutation.BloopyId(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeUUID,
			Value:  value,
			Column: user.FieldBloopyId,
		})
		_node.BloopyId = value
	}
	...
	if value, ok := uc.mutation.Plexid(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
	...
	if value, ok := uc.mutation.Bloopnetid(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
			Column: user.FieldBloopnetid,
		})
		_node.Bloopnetid = value
	}
	if value, ok := uc.mutation.Authentikpkid(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
			Column: user.FieldAuthentikpkid,
		})
		_node.Authentikpkid = value
	}
	...
	if nodes := uc.mutation.MediaRequestIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
