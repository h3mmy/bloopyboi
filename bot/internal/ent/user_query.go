...
func (uq *UserQuery) QueryMediaRequest() *MediaRequestQuery {
	query := &MediaRequestQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) loadMediaRequest(ctx context.Context, query *MediaRequestQuery, nodes []*User, init func(*User), assign func(*User, *MediaRequest)) error {
	...
	query.Where(predicate.MediaRequest(func(s *sql.Selector) {
		s.Where(sql.InValues(user.MediaRequestColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_media_request" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
