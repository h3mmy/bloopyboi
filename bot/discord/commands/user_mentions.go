...
func DirectedMessageReceive(s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	if !directMessage && botMentioned {
		logger.Debug(fmt.Sprintf("Detected Channel Mention in message from %s with UserID %s and Content: ", m.Author.Username, m.Author.ID), m.Content)
		...
		logger.Debug("Detected Direct Message from ", m.Author.Username)
