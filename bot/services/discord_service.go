...
func (d *DiscordService) RecordNewMessage(ctx context.Context, m *discordgo.Message) error {
	...
	// save to DB
	return d.db.DiscordMessage.Create().
		SetAuthor(discUser).
		SetDiscordid(m.ID).
		SetGuild(guild).
		SetRaw(*m).
...
func (d *DiscordService) GetSavedDiscordUser(ctx context.Context, discordUserID string) (*ent.DiscordUser, error) {
	...
	if err != nil {
		if ent.IsNotFound(err) {
			...
			err = d.saveDiscordUser(ctx, discordUser)
			if err != nil {
				return nil, err
			}
