...
func (uq *UserQuery) QueryMediaRequest() *MediaRequestQuery {
	query := (&MediaRequestClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, user.MediaRequestTable, user.MediaRequestColumn),
		)
		fromU = sqlgraph.SetNeighbors(uq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withMediaRequest: uq.withMediaRequest.Clone(),
...
// the "mediaRequest" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithMediaRequest(opts ...func(*MediaRequestQuery)) *UserQuery {
	query := (&MediaRequestClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withMediaRequest = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withMediaRequest; query != nil {
		if err := uq.loadMediaRequest(ctx, query, nodes,
			func(n *User) { n.Edges.MediaRequest = []*MediaRequest{} },
			func(n *User, e *MediaRequest) { n.Edges.MediaRequest = append(n.Edges.MediaRequest, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadMediaRequest(ctx context.Context, query *MediaRequestQuery, nodes []*User, init func(*User), assign func(*User, *MediaRequest)) error {
	...
	query.Where(predicate.MediaRequest(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.MediaRequestColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_media_request
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_media_request" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_media_request" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
